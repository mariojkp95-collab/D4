// build split-1
console.log('[GAME] game.js caricato');
<!doctype html>
<html lang="it">
<head>
<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Drakoria — HUD-4</title>
<style>
:root{
...
<!-- Mini mappa -->
<div id="miniWrap" class="hidden"><canvas id="mini" width="80" height="48" aria-label="Mini mappa"></canvas></div>
...
(function(){
  ...
  try{
    ...
    // Inventario & equip
    var inventory=[]; // {id,name,icon,rarity,type,qty?, slot?}
    var equipped={weapon:null, armor:null};

    // Categorie inventario (stile NosTale)
    var INV_TABS=['Tutto','Armi','Armature','Consumabili','Materiali'];
    var currentTab='Tutto';

    // Hotbar binding (slot -> binding)
    // binding: { kind:'potion'|'item', icon:'🍵', itemId?:number, type?:string }
    ...
    function tileFree(x,y){
      ...
      if(Array.isArray(lootBags)){ for(i=0;i<lootBags.length;i++){ if(lootBags[i].x===x && lootBags[i].y===y) return false; } }
      return true;
    }
    function randEmpty(){ for(var k=0;k<500;k++){ var x=(Math.random()*COLS|0), y=(Math.random()*ROWS|0); if(inside(x,y) && tileFree(x,y)) return {x:x,y:y}; } return {x:2,y:2}; }

    // PATHFIND + helpers (brevi)
    ...
    cv.addEventListener('click',function(ev){
      ...
      if(clickedEnemy){
        if(lastEnemyTapId===clickedEnemy.id && (now-lastEnemyTapTs)<=350){ target = clickedEnemy; autoChase = true; pathQueue = []; }
        else { target = clickedEnemy; autoChase = false; }
    ...
    function openLootIfAny(){
      ...
      var commonOnly = true;
      for (var i = 0; i < bag.items.length; i++) { var t = bag.items[i].type; if (t !== 'coin' && t !== 'potion') { commonOnly = false; break; } }
      if (commonOnly) {
        var beforeCoins = player.coins, beforePots = player.pots;
        var emptied = autoPickupCommons(bag);
        var coinsGained = player.coins - beforeCoins;
        var potsGained  = player.pots  - beforePots;
        ...
        if (coinsGained || potsGained) { toast('Raccolto: ' + (coinsGained?('🪙 x'+coinsGained+' '):'') + (potsGained?('🍵 x'+potsGained):'')); if(coinsGained) logLoot('Monete +' + coinsGained); if(potsGained) logLoot('Pozioni +' + potsGained); }
        return;
      }
      renderLoot(bag); lootWin.classList.remove('hidden');
    }
    function getBagAt(x,y){ for(var i=0;i<lootBags.length;i++){ if(lootBags[i].x===x && lootBags[i].y===y) return lootBags[i]; } return null; }
    function renderLoot(bag){
      ...
      for(var i=0;i<items.length;i++){
        (function(it){
          ...
          d.innerHTML='<span class="lbl">'+ (it.qty?('x'+it.qty):'') +'</span><div class="ico">'+it.icon+'</div><div class="nm">'+it.name+'</div>';
    ...
    function applyItem(it){
      if(it.type==='coin'){ player.coins += it.qty; questAdd('coin', it.qty); logLoot('Monete +' + it.qty); }
      else if(it.type==='potion'){ player.pots += it.qty; logLoot('Pozioni +' + it.qty); }
      else { inventoryStackOrPush(it); renderInventoryIfOpen(); toast('Ottenuto: '+it.name+(it.qty?(' x'+it.qty):'')); logLoot('Ottenuto: '+it.name); }
    }
    function inventoryStackOrPush(it){
      if(typeof it.qty==='number'){
        for(var i=0;i<inventory.length;i++){ var x=inventory[i]; if(x.type===it.type && x.name===it.name && typeof x.qty==='number'){ x.qty+=it.qty; return; } }
      }
      inventory.push(it);
    }
    function renderInventoryIfOpen(){ if(!invWin.classList.contains('hidden')) { renderTabs(); renderInventory(); } }
    function renderTabs(){
      invTabs.innerHTML='';
      INV_TABS.forEach(function(t){
        var b=document.createElement('button'); b.className='tab'+(currentTab===t?' active':''); b.textContent=t;
        b.onclick=function(){ currentTab=t; renderInventory(); };
        invTabs.appendChild(b);
      });
    }
    function typeToTab(t){
      if(t==='weapon') return 'Armi';
      if(t==='armor') return 'Armature';
      if(t==='potion' || t==='consumable') return 'Consumabili';
      if(t==='material') return 'Materiali';
      return 'Tutto';
    }
    function renderInventory(){
      eqWeaponName.textContent = equipped.weapon ? (equipped.weapon.icon+' '+equipped.weapon.name+' (+'+ (equipped.weapon.atkBonus||0) +' ATK)') : '—';
      eqArmorName.textContent  = equipped.armor  ? (equipped.armor.icon +' '+equipped.armor.name +' (+'+ (equipped.armor.hpBonus||0) +' HP)') : '—';
      ...
      var items = inventory.filter(function(it){
        ...
        return typeToTab(it.type)===currentTab;
      });

      if(items.length===0){ invGrid.innerHTML='<div style="grid-column:1/-1;text-align:center;color:#9aa3b2">Nessun oggetto</div>'; return; }

      for(var i=0;i<items.length;i++){
        (function(it){
          var d=document.createElement('div'); d.className='invItem '+rarityClass(it.rarity);
          var qty = (typeof it.qty==='number' ? (' ×'+it.qty) : '');
          d.innerHTML = '<div class="cap">'+(it.icon||'')+' '+it.name+qty+'</div>';

          var row=document.createElement('div'); row.className='row';

          // Equip/Usa
          if(it.type==='weapon'){
            var b1=document.createElement('button'); b1.className='act'; b1.textContent='Equippa 🗡️ (+'+(it.atkBonus||0)+' ATK)';
            b1.onclick=function(){ equip(it,'weapon'); removeFromInventoryById(it.id); renderInventory(); toast('Equipaggiata: '+it.name); logSys('Equip: '+it.name); };
            row.appendChild(b1);
          } else if(it.type==='armor'){
            var b2=document.createElement('button'); b2.className='act'; b2.textContent='Equippa 🛡️ (+'+(it.hpBonus||0)+' HP)';
            b2.onclick=function(){ equip(it,'armor'); removeFromInventoryById(it.id); renderInventory(); toast('Equipaggiata: '+it.name); logSys('Equip: '+it.name); };
            row.appendChild(b2);
          } else if(it.type==='potion' || it.type==='consumable'){
            var b3=document.createElement('button'); b3.className='act'; b3.textContent='Usa';
            b3.onclick=function(){
              if(it.heal){ var before=player.hp; player.hp=Math.min(player.maxHp, player.hp+it.heal); var healed=player.hp-before; if(healed>0){ logHeal('Consumabile: +'+healed+' HP.'); flashScreen('#8b5cf6'); draw(); } }
              if(typeof it.qty==='number'){ it.qty--; if(it.qty<=0) removeFromInventoryById(it.id); renderInventory(); }
            };
            row.appendChild(b3);
          }
          ...
          // Assegna a barra
          if(it.type==='potion' || it.type==='consumable'){
            var b4=document.createElement('button'); b4.className='act'; b4.textContent='→ Barra';
            b4.onclick=function(){ openSlotPicker(function(slot){ assignToHotbar(slot, it); }); };
            row.appendChild(b4);
          }
    ...
    function removeFromInventoryById(id){
      for(var i=0;i<inventory.length;i++){ if(inventory[i].id===id){ inventory.splice(i,1); return; } }
    }
    function removeFromInventory(index){ if(index>=0 && index<inventory.length) inventory.splice(index,1); }
    function equip(item, slot){
      if(slot==='weapon' && equipped.weapon) inventory.push(equipped.weapon);
      if(slot==='armor'  && equipped.armor ) inventory.push(equipped.armor);
      equipped[slot]=item; recomputeStats(false); draw();
    }
    function unequip(slot){
      if(!equipped[slot]) return;
      inventory.push(equipped[slot]); equipped[slot]=null; recomputeStats(false); renderInventory(); draw(); toast('Rimosso equip.');
    }

    // Slot picker & hotbar assign
    function openSlotPicker(cb){
      slotPicker.classList.remove('hidden');
      var btns=slotPicker.querySelectorAll('.pickBtn');
      function onPick(ev){
        var s=+ev.currentTarget.getAttribute('data-slot');
        for(var i=0;i<btns.length;i++) btns[i].removeEventListener('click', onPick);
        slotPicker.classList.add('hidden');
        cb(s);
      }
      for(var i=0;i<btns.length;i++) btns[i].addEventListener('click', onPick, false);
    }
    function assignToHotbar(slot, it){
      hotbar[slot]={kind:'item', icon:it.icon||'❖', itemId:it.id, type:it.type};
      capEls[slot].textContent = it.icon||'❖';
      toast(it.name+' assegnato allo slot '+slot);
    }
    ...
    btnLoad.addEventListener('click', function(){ try{
      ...
      if(d && d.hotbar){ hotbar=d.hotbar; for(var s=1;s<=5;s++){ capEls[s].textContent = hotbar[s] ? (hotbar[s].icon||'—') : '—'; } }
      ...
      renderQuest(); updateHotbarCounts(); draw(); drawMini(); renderInventoryIfOpen(); updateTargetFrame(); updateDpsPanel(); renderAbilities();
      toast('Caricato.');
          ...
          if(e.aggro){
            if(e.type==='melee'){
              ...
              if(inRange){
                ...
                if(canMove){
                  if(distM>e.rangeMax || !hasLoS(e.x,e.y,player.x,player.y)){
                    e.path = bfs(e.x,e.y,player.x,player.y) || e.path || [];
                    tryStepAlongPath(e);
                  }
                }
              }
...
(function(){
  ...
  function injectCss(){
    if(document.getElementById('patch01-css')) return;
  ...
  function startCd(){
    ...
    if(s1) s1.classList.add('cooldown');
    raf();
  }
  function raf(){
    if(!cdEnd) return;
  ...
  function setup(){
    ...
    const fab=document.getElementById('btnUsePotion');

    // Avvio cooldown SOLO se il contatore pozioni è sceso dopo il tap
    function tryStartAfterUse(){
      ...
      setTimeout(function(){
        ...
        if(after!==before){ startCd(); } // usata davvero
      }, 40);
    }

    slot1?.addEventListener('click', tryStartAfterUse, true);
    fab?.addEventListener('click', tryStartAfterUse, true);
    fab?.addEventListener('touchend', tryStartAfterUse, {passive:true});
